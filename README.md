...
You can customize the Quick Add menu with different function calls inside and out of the `quick\_add\_menu.gd` `add\_custom\_items()` function.

The best place to start is inside of the `add\_custom\_items()` function if you do not have your own editor plugin.
...
```gdscript
func add\_custom\_items(): ## Add custom items
    var header = QuickAddMenu.Item.new\_header("This is a cool header!")
    QuickAddMenu.node\_3d\_list.append(header)
```
